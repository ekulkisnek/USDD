...
pub struct ProtocolManifest {
    ...
    pub elements_guest_program_id: Hash32,
...
impl ProtocolManifest {
    pub fn validate(&self) -> Result<(), ManifestError> {
        ...
        for (name, hash) in [
            ...
            ("elements_guest_program_id", self.elements_guest_program_id),
            (
    ...
    pub fn compute_vault_id(&self) -> Hash32 {
        ...
        self.elements_guest_program_id.encode_to(&mut preimage);
    ...
    /// hash). Fields that may depend on the controller CMR or bootstrap
    /// issuance are intentionally frozen elsewhere in the full manifest.
...
impl CanonicalEncode for ProtocolManifest {
    fn encode_to(&self, out: &mut Vec<u8>) {
        ...
        self.elements_guest_program_id.encode_to(out);
...
impl CanonicalDecode for ProtocolManifest {
    fn decode_from(decoder: &mut Decoder<'_>) -> Result<Self, DecodeError> {
        ...
        let value = Self {
            ...
            elements_guest_program_id: Hash32::decode_from(decoder)?,
...
mod tests {
    ...
    fn controller_configuration_is_non_circular_and_enforced() {
        let manifest = valid_manifest();
        assert_eq!(
            manifest.controller_configuration_hash,
            manifest.compute_controller_configuration_hash()
        );
